//im trynna prove that maj and inv have a joint symmetric distribution.
#include <bits/stdc++.h>
using namespace std;
using vi=vector<int>;
...
int main()
{
    ...
    {   int a, b;
        ...
        sort(f[a][b].begin(),f[a][b].end(),[=](vi aa, vi bb){return (aa[n-1]<bb[n-1]);});
        cout<<f[a][b].size()<<endl;
        for (vi v: f[a][b])
        {
            for (int l:v)
            {cout<<l;}
